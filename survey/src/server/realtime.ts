...
/** Read-only monitor channel on /ws. HTTP GET /api/city-state stays the recovery path after reconnects. */
export function attachRealtime(server: Server, currentState: () => CitySurveyState) {
  ...
  server.on('upgrade', (request: IncomingMessage, socket: Duplex, head: Buffer) => {
    ...
    wss.handleUpgrade(request, socket, head, client => {
      ...
      send(client, { type: 'city-state-snapshot', state: currentState() });
    });
  });
  return {
