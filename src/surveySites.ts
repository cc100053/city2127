...
export function surveySites(scene: T.Scene) {
  ...
  // Tall variant: a round residential tower in the ringed language (ART.md §2) — glass core, ceramic spandrels,
  // trim floor lips and a planted balcony ring every third floor, under a roof garden. Distinct from the NW glass shaft.
  const core = new T.Mesh(new T.CylinderGeometry(3, 3, 26, 40), glass); core.position.y = 13; towerUpper.add(core);
  for (let y = 0, floor = 0; y < 25; y += 2.6, floor++) {
    arc(towerUpper, 0, 3.2, 1, [0, y, 0], cream);
    arc(towerUpper, 0, 3.45, .14, [0, y + 1, 0], trim);
    if (floor % 3 === 1) { arc(towerUpper, 3.05, 3.45, .3, [0, y + 1.14, 0], leaf); shrubs(towerUpper, 3.25, y + 1.2, 0, Math.PI * 2, 16); }
  }
