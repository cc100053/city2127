...
使用者決定（2026-09-25）：root 場景取消三個狀態按鈕，改為自動日夜循環（一日 3 分鐘）。Still／Daylight／Pulse 變成清晨／日間／夜晚嘅氣氛，夜晚唔再限定日光。詳見 [PROJECT](PROJECT.md#current-prototype-behavior-not-the-exhibition-specification)。
...
歷史 Plan 02 美術驗收、完整網格碰撞及 1080p 效能仍有未驗證項目，詳見下方原始紀錄。現行展覽真正待辦是上方的題目／選項、累積映射、體驗交接與重設等設計及後續實作；新展覽流程尚未實作。舊美術清單不是它們的前置門檻。
