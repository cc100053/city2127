...
Frame flow: `main` reads the day clock (`hourAt`, or `?hour`) → the city state is `moodAt(hour)` (or, in survey mode, `worldState` blending toward the survey target), passed through `withNight()` → sky, fog, sun/moon and fill are lit for that hour → `cityRig.update(state,time)` updates materials, windows and membranes and calls mobility → survey sites update → the overlay shows the clock → the composer renders. One clock drives all motion.
