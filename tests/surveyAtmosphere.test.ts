...
import { parseSurveyEvent, scoresToWorldState, siteTargets, supersedes } from '../src/surveyAtmosphere.ts';
...
const view = { runId: 'r', revision: 1, scores: { ...zero, automation: 2 }, layout, history: [{ revision: 1, optionLabel: 'x', policyChange: { automation: 2 }, cityChanges: [{ socketId: 'nw', label: 'hub' }] }] };
...
console.log('PASS: survey scores → atmosphere mapping, layout → change sites, event parsing, revision order, blendTo.');
